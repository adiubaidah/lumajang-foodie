...
function ReviewPlace() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["place-review", { user: params.id }],
    queryFn: async () => {
      return axiosInstance
        .get(`/place-review?user=${params.id}`)
        .then((data) => data.data);
    },
    staleTime: 1000 * 5 * 60,
    enabled: !!params.id,
    ...
    <>
      {isLoading || !data ? (
        ...
        data.result.map((review: Review) => (
          <div
            key={review.id}
            className="w-full border-b-[1px] border-b-gray-300 py-3"
          >
            ...
            <p className="mt-3 font-helvetica text-[16px] text-gray-700">
              {review.review}
            </p>
          </div>
        ))
